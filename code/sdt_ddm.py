...
def apply_hierarchical_sdt_model(data):
    ...
    with pm.Model() as sdt_model:
        ...
        # Hyperpriors for criterion intercept and fixed effects
        intercept_criterion = pm.Normal('intercept_criterion', mu=0, sigma=1)
        stim_criterion = pm.Normal('stim_criterion', mu=0, sigma=1)
        diff_criterion = pm.Normal('diff_criterion', mu=0, sigma=1)
        interaction_criterion = pm.Normal('interaction_criterion', mu=0, sigma=1)
...
def analyze_data(file_path):
    ...
    with apply_hierarchical_sdt_model(sdt_data) as model:
        ...
        trace_vars = ["intercept_d_prime", "stim_d_prime", "diff_d_prime", "interaction_d_prime", "intercept_criterion", "stim_criterion", "diff_criterion", "interaction_criterion"]
...
if __name__ == "__main__":
    trace, dp_data = analyze_data("/home/jovyan/107-final/data/data.csv")
